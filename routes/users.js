...
router.post("/signUp", async (req, res, next) => {
  try {
    ...
    if (!user) {
      ...
      res.status(200).send({ message: "User Signup Successfull !!!", token });
    } else {
    ...
    if (user) {
      if (user.activation === true) {
        if (await comparePassword(req.body.password, user.password)) {
          ...
          res
            .status(200)
            .send({ message: "User Login Successfull !!!", token });
        } else {
        ...
        res.status(402).send({message:"Please Activate Your Account, An Email Already Sent To Your Mail Id.",token});
      }
    } else {
